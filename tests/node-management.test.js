...
describe('deleteNodePure', () => {
  ...
  // Document known behavior: deleteNodePure does NOT remove back-references
  // from connected nodes (matches app behavior in deleteNode()).
  it('does NOT clean up back-references from connected nodes (known limitation)', () => {
    const nodes = makeNodes(3);
    nodes[0].connections = ['N02'];
    nodes[1].connections = ['N01', 'N03'];
    nodes[2].connections = ['N02'];
    ...
    deleteNodePure(nodes, 1); // delete N02
    // Old N03 (now N02) still has its old connection reference, even though
    // the node it pointed to was deleted.
